...
# Tool names stay snake_case whatever convention an upstream spec names operations by.
_CAMEL_BOUNDARY = re.compile(r"(?<=[a-z0-9])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])")

# Separates the name FastAPI generates for an undeclared operation_id from its suffix.
_AUTOGENERATED_SEPARATOR = "__"
...
def tool_name(operation_id: str) -> str:
    stem = operation_id.split(_AUTOGENERATED_SEPARATOR)[0]
    return _CAMEL_BOUNDARY.sub("_", stem).lower()


def tool_names(spec: dict) -> dict[str, str]:
    """Map every flagged operationId to its tool name, for FastMCP's ``mcp_names``."""
    return {
        operation["operationId"]: tool_name(operation["operationId"])
